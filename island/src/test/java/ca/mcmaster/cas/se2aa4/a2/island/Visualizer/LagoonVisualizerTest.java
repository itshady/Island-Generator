...
class LagoonVisualizerTest {
    ...
    public void LagoonTest() {
        ...
        assertEquals(new Color(103,168,209,255), tile.getPolygon().getColor());
    }

    @Test
    public void LowAbsorptionVisualizeTest() {
        Color anyColor = new Color(110,10,140,255);
        tile.setColor(anyColor);
        new LagoonVisualizer().process(island);

        assertEquals(anyColor, tile.getPolygon().getColor());
    }
}