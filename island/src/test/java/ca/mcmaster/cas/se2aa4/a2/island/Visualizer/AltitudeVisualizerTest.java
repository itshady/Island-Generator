...
class AltitudeVisualizerTest {
    ...
    @Test
    public void HighAltitudeVisualizeTest() {
        tile.setAltitude(ElevationUtil.maxAltitude);
        new AltitudeVisualizer().process(island);

        assertEquals(new Color(233, 62, 58), tile.getPolygon().getColor());
    }

    @Test
    public void LowAltitudeVisualizeTest() {
        tile.setAltitude(ElevationUtil.minAltitude);
        new AltitudeVisualizer().process(island);

        assertEquals(new Color(255, 243, 59), tile.getPolygon().getColor());
    }
}