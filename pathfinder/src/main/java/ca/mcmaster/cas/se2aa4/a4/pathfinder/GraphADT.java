...
public class GraphADT {
    ...
    public void addEdge(Integer n1, Integer n2, Double weight, Integer id) throws NoSuchNodeExists {
    ...
    public void addEdge(Integer n1, Integer n2, Integer weight, Integer id) throws NoSuchNodeExists {
        Node node1 = getNode(n1);
        Node node2 = getNode(n2);
        if (node1 == null) throw new NoSuchNodeExists("Node id "+n1+" does not exist.");
        if (node2 == null) throw new NoSuchNodeExists("Node id "+n2+" does not exist.");
        Edge edge = new Edge(node1, node2, weight*1.0, id);
        adjacencyList.get(node1).add(edge);

        if (!directed) {
            Edge oppositeEdge = new Edge(node2, node1, weight*1.0, id);
            adjacencyList.get(node2).add(oppositeEdge);
        }
    }
